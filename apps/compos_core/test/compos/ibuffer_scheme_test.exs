defmodule Compos.IbufferSchemeTest do
  @moduledoc """
  Runs priv/tests/ibuffer-test.scm alone: sections, order, folds, ages.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "ibuffer-test.scm"])
  ...
  @tag timeout: 120_000
  test "ibuffer-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
