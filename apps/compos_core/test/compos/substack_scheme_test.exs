defmodule Compos.SubstackSchemeTest do
  @moduledoc """
  Runs priv/tests/substack-test.scm alone: subscription parsing and the three app modes.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "substack-test.scm"])
  ...
  @tag timeout: 300_000
  test "substack-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
