defmodule Compos.SubagentsSchemeTest do
  @moduledoc """
  Runs priv/tests/subagents-test.scm alone: the spawn edges as a table.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "subagents-test.scm"])
  ...
  @tag timeout: 180_000
  test "subagents-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
