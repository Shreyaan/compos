defmodule Compos.HandheldSchemeTest do
  @moduledoc """
  Runs priv/tests/handheld-test.scm alone: the handheld client's policy.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "handheld-test.scm"])
  ...
  @tag timeout: 180_000
  test "handheld-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
