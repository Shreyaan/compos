defmodule Compos.ProfileSchemeTest do
  @moduledoc """
  Runs priv/tests/profile-test.scm alone: the one-command profiler's policy.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "profile-test.scm"])
  ...
  @tag timeout: 300_000
  test "profile-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
