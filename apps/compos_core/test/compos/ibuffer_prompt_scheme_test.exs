defmodule Compos.IbufferPromptSchemeTest do
  @moduledoc """
  Runs priv/tests/ibuffer-prompt-test.scm alone: the table in the
  minibuffer form for buffers and chats.
  """
  ...
  @file_ Path.join([:code.priv_dir(:compos_core), "tests", "ibuffer-prompt-test.scm"])
  ...
  @tag timeout: 120_000
  test "ibuffer-prompt-test.scm passes" do
    eval!(~s{(load "#{@file_}")})
    names = names()
