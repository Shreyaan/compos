...
(define (group-cycle-test-reset!)
  (when (boundp '*hidden-windows*)
    (set! *hidden-windows*
      (filter (lambda (r)
                (null? (filter (lambda (b) (string-prefix? "*zz-cyc-" b)) (nth 3 r))))
              *hidden-windows*)))
...
(deftest 'mode-consolidate-gathers-history-and-preserves-unrelated-work
  ...
  (lambda ()
    (let* ((windows (group-consolidate-test-open!))
      ...
      (check-equal! (length (hidden-window-list)) 1 "one invisible window holds the other stack")
      (check-equal! (hidden-window-buffers (car (hidden-window-list))) '("*zz-cyc-work-there*")
                    "the invisible window contains the other stack")
      ...
      (let ((before (window-tree)) (hidden *hidden-windows*))
        (run-command "mode-consolidate")
        (check-equal! (window-tree) before "a second consolidation changes nothing")
        (check-equal! *hidden-windows* hidden "a second consolidation creates no hidden windows")))
    (group-cycle-test-reset!)))
...
(deftest 'mode-consolidate-keeps-the-other-stack-invisible-at-target-capacity
  ...
  (lambda ()
    ...
    (let ((destination (window-showing "*zz-cyc-work-a*"))
      ...
      (check-equal! (hidden-window-buffers (car (hidden-window-list)))
                    '("*zz-cyc-one*" "*zz-cyc-two*") "the invisible stack keeps its order")
