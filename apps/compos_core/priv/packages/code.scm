...
(defcustom 'code-instructions
  (string-append
    "Follow the requested scope. For a direct tool action or read-only query, "
    "call the tool and return its result; do not inspect or edit the repository. "
    "For a code change, inspect, edit, and verify. Use (code-outline \"BUF\") "
    "and (code-find \"BUF\" \"text\") to locate definitions, "
    "(code-read \"BUF\" LINE) to read one, and "
    "(code-replace! \"BUF\" LINE NEW) to replace one. For an expression use "
    "(code-sexp \"BUF\" \"anchor\" [LEVELS]) and "
    "(code-sexp-replace! \"BUF\" \"anchor\" NEW [LEVELS]). For text edits use "
    ...
    "(buffer-delete-text! \"BUF\" TEXT). Read before editing, save the file, "
    "and read the result back. Keep changes under (default-directory). "
    "The browser category is denied in code-mode by default. Verify editor "
    "UI through compos state and key dispatch. If that cannot suffice, explain "
    "why and ask the user to enable M-x browser-mode.")
