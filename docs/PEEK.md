...
8. `M-<down>` and `M-<up>` from the listing scroll the peek: `scroll-other-window` reads the look beside your work (a peek, the messages, the telemetry) before the next window. `scroll-popup` and `scroll-popup-down` scroll the popup by name, for a binding of your own.
